...
def generate_prompts(high_sim_samples, k=5):
    ...
    for t in tqdm(batchify_dataframe(high_sim_samples, k), total=total_batches, desc='Generating prompts'):
        ...
        k_i = min(k, len(t))
        prompt = (f"""I will show you {k_i} pairs of descriptions, all from different news articles.

        Are the two descriptions describing similar narrative roles in their respective articles?
        Answer with "Yes" or "No". Answer each sequentially and number them with 1., 2., 3., etc.

        [Short Example]
        1. Description 1: ```Event Detail: This sentence provides specific details about a particular event or action within the larger narrative of the news article.```, Description 2: ```Background Information: This sentence provides additional context about that is relevant to the current discussion, but is not the main focus of the article.```
        2. Description 1: ```"Elaboration": This sentence provides additional details about the process of exploring music, explaining how it can lead to a deeper understanding of the music\'s background.```, Description 2: ```"Development": This sentence provides additional information about a person\'s understanding or perspective, adding depth to their character or role in the story.```
